...
test('owner approval binds exact graph and projection, then proposals retain complete trace links', async () => {
  ...
  assert.throws(
    () => staleIntentService.proposeSolution([{
      name: 'Text analysis',
      intentNodeIds: [discovered.intentGraph.nodes[0].id],
    }]),
    (error) => error instanceof ProjectServiceError && error.code === 'missing_baseline',
  );

  assert.throws(
    () => service.proposeSolution([{ name: 'Untraced', intentNodeIds: [] }]),
    (error) => error instanceof ProjectServiceError && error.code === 'invalid_trace',
  );
  const solution = service.proposeSolution([{
    name: 'Text analysis',
    intentNodeIds: [discovered.intentGraph.nodes[0].id],
  }]);
  ...
  assert.equal(execution.nodes.length, solution.nodes.length * 3);
  assert.equal(execution.crossGraphLinks.length, execution.nodes.length);
