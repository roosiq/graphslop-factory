...
const validFixtures = [
  ...
  [NodeRefSchema, { graphKind: 'evidence', nodeId: 'node-1' }],
...
test('common contracts reject invalid and unknown-field fixtures', () => {
  ...
  assert.equal(NodeRefSchema.safeParse({ graphKind: 'evidence', nodeId: 'node-1', extra: true }).success, false);
...
test('common contract registry exposes portable strict schemas', () => {
  ...
  assert.deepEqual(roundTripped.schemas.nodeRef.required, ['graphKind', 'nodeId']);
