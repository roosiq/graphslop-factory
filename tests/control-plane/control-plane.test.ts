...
describe('control plane boundary', () => {
  test('reports the local model and downloads the owner build pack as a zip', async () => {
    const sessions = new SoleOwnerSessions();
  ...
  test('opening the local workspace sets strict cookies and rotates the browser session', async () => {
    const sessions = new SoleOwnerSessions();
    ...
    const rotated = await app.request('http://127.0.0.1/api/v1/auth/session', {
      headers: { host: '127.0.0.1' },
    });
    expect(rotated.status).toBe(201);
    const oldSession = await app.request('http://127.0.0.1/api/v1/owner/project', {
      headers: { host: '127.0.0.1', cookie: cookies(first) },
    });
  ...
  test('rejects public host and mismatched origin', async () => {
    const app = createControlPlane({
      sessions: new SoleOwnerSessions(),
      adapter: new MemoryControlAdapter(null, envelope.bindings),
      runnerToken,
    });
...
describe('production adapter', () => {
  ...
  test('actual ProjectService authority dispatches through actual LocalRunner and then rejects stale binding', async () => {
    ...
    const intentNodeId = project.state().intentGraph!.nodes[0]!.id;
    project.proposeSolution({
      features: [{ key: 'feature-one', name: 'Feature one', intentNodeIds: [intentNodeId] }],
      roles: [{
        key: 'feature-engineer',
        name: 'Feature engineer',
        intentNodeIds: [intentNodeId],
        job: 'Make feature one.',
        use: ['Approved need.'],
        touch: ['Feature one.'],
        dont: ['Change intent.'],
        done: ['Checks pass.'],
      }, {
        key: 'feature-reviewer',
        name: 'Feature reviewer',
        intentNodeIds: [intentNodeId],
        job: 'Check feature one.',
        use: ['Approved need and result.'],
        touch: ['Verification only.'],
        dont: ['Build feature one.'],
        done: ['Report pass or gap.'],
      }],
      assignments: [{
        featureKey: 'feature-one',
        roleKey: 'feature-engineer',
        taskTypes: ['Decide', 'Implement'],
      }, {
        featureKey: 'feature-one',
        roleKey: 'feature-reviewer',
        taskTypes: ['Verify'],
      }],
    });
    const solutionProjection = project.createReviewProjection('solution');
    const projectedSolution = solutionProjection.data as {
      nodes: { type?: string; attributes?: Record<string, unknown> }[];
      edges: { type: string; attributes: Record<string, unknown> }[];
      traces: { type: string }[];
    };
    expect(projectedSolution.nodes.find((node) => node.type === 'Role')?.attributes?.job)
      .toBe('Make feature one.');
    expect(projectedSolution.edges.find((edge) => edge.type === 'USES')?.attributes.taskTypes)
      .toEqual(['Decide', 'Implement']);
    expect(projectedSolution.traces.some((trace) => trace.type === 'SATISFIES_INTENT')).toBe(true);
  ...
  test('real HTTP commands drive CAPTURE through Execution compilation without out-of-band lifecycle calls', async () => {
    ...
    const project = new ProjectService({
      ...
      planSolution: async ({ intentNodes }) => ({
        features: [{ key: 'feature', name: 'Feature', intentNodeIds: [intentNodes[0]!.id] }],
        roles: [{
          key: 'feature-engineer',
          name: 'Feature engineer',
          intentNodeIds: [intentNodes[0]!.id],
          job: 'Make the feature.',
          use: ['Approved need.'],
          touch: ['Feature.'],
          dont: ['Change intent.'],
          done: ['Checks pass.'],
        ...
        assignments: [{
          featureKey: 'feature',
          roleKey: 'feature-engineer',
          taskTypes: ['Decide', 'Implement'] as ('Decide' | 'Implement' | 'Verify')[],
        }, {
          featureKey: 'feature',
          roleKey: 'feature-reviewer',
