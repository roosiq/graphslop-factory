...
export type CodexProposalCall = (prompt: string) => Promise<unknown>;
...
export class LocalQwenClient {
  ...
  async call(prompt: string): Promise<unknown> {
    const model = await this.model();
 ...
 */
export class CodexProposalProvider implements ProposalProvider {
  ...
  async propose(context: ProposalContext): Promise<ProposalOutput> {
    ...
    return ProposalOutputSchema.parse(normalizeProposal(await this.call(prompt), context));
  }
}
