...
function loadAuthoritySecrets(stateRoot: string): AuthoritySecrets {
  const path = resolve(stateRoot, 'authority-secrets.json');
  try {
    const value = JSON.parse(readFileSync(path, 'utf8')) as AuthoritySecrets;
    if (value.version !== 1 || [value.lease, value.worker, value.drift, value.runner]
...
export function startLocalProduct(input: Readonly<{
  ...
  const secrets = loadAuthoritySecrets(stateRoot);
  ...
  const proposalProvider = testMode
    ? {
      ...
      planSolution: async ({ intentNodes }: {
        intentNodes: readonly { id: string; statement: string }[];
      }) => ({
        features: [{
          key: 'approved-product',
          name: intentNodes[0]?.statement ?? 'Approved product',
          intentNodeIds: intentNodes.map((node) => node.id),
        }],
        roles: [{
          key: 'product-maker',
          name: 'Product maker',
          intentNodeIds: intentNodes.map((node) => node.id),
          job: 'Make the approved thing.',
          use: ['Approved needs.'],
          touch: ['Only the assigned feature.'],
          dont: ['Change the need.'],
          done: ['Feature works and checks pass.'],
        }, {
          key: 'product-checker',
          name: 'Product checker',
          intentNodeIds: intentNodes.map((node) => node.id),
          job: 'Check the approved thing.',
          use: ['Approved needs and built result.'],
          touch: ['Only verification.'],
          dont: ['Build the feature being checked.'],
          done: ['Report pass or exact gap.'],
        }],
        assignments: [{
          featureKey: 'approved-product',
          roleKey: 'product-maker',
          ...
          task: {
            ...
            brief: {
              ...
              dont: ['Change intent.', 'Add scope.', 'Push or deploy.', ...(
                ...
                Array.isArray(roleNode.attributes.done) ? roleNode.attributes.done.map(String) : []
              )].join(' '),
            },
            solutionNodeIds: [String(task.attributes.solutionNodeId), roleNode.id],
