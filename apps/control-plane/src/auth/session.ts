...
export class SoleOwnerSessions {
  ...
  constructor(claimToken = opaqueToken()) {
    if (claimToken.length < 24) throw new Error('Claim token is too short.');
    this.claimToken = claimToken;
  }

  claim(candidate: string): OwnerSession | undefined {
    if (this.#claimed || !sameSecret(candidate, this.claimToken)) return undefined;
    this.#claimed = true;
