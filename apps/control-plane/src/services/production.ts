...
export interface ProjectServiceBoundary {
  ...
  proposeSolution(features: readonly { name: string; intentNodeIds: readonly string[] }[]): unknown;
  ...
  private async execute(command: OwnerCommand, input: unknown): Promise<unknown> {
    ...
    switch (command) {
      ...
      case 'propose-solution':
        if (!Array.isArray(value.features)) throw new Error('Solution features are required.');
        return this.project.proposeSolution(value.features as readonly { name: string; intentNodeIds: readonly string[] }[]);
