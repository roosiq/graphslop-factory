...
export function createControlPlane(options: ControlPlaneOptions): Hono {
  ...
  app.post('/api/v1/auth/claim', async (context) => {
    const body = await jsonBody(context.req.raw) as Record<string, unknown>;
    const claimed = typeof body?.claimToken === 'string' ? options.sessions.claim(body.claimToken) : undefined;
    if (!claimed) return context.json(error('claim_denied', 'Claim is invalid or already used.'), 409);
    for (const cookie of ownerCookies(claimed, effectiveProtocol(context.req.raw) === 'https:')) {
      context.header('Set-Cookie', cookie, { append: true });
    }
    return context.json({ claimed: true, csrfToken: claimed.csrf }, 201);
  });
